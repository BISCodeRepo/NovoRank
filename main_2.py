...
if config['TRAIN'] == True:
    ...
    train, val = train_val_split(new_data_, config['VAL_SIZE'])
    ...
    data_train = tf.data.Dataset.from_generator(lambda: generator_train(train, config['MGF_PATH']),
    ...
    data_val = tf.data.Dataset.from_generator(lambda: generator_val(val, config['MGF_PATH']),
                                              output_types=({'input_spec': tf.float64, 'input_seq_1': tf.float64, 'input_seq_2': tf.float64,
                                                             'input_add_1': tf.float64, 'input_add_2': tf.float64, 'input_add_add': tf.float64},
                                                            tf.float64),
                                              output_shapes=output_shape)

    data_train = data_train.batch(config['BATCH'])
    data_val = data_val.batch(config['BATCH'])
    ...
    pre_trained_model = tf.keras.models.load_model(PATH + '\\' + config['PRE_TRAINED_MODEL'])

    output_shape = ({'input_spec': [50000], 'input_seq_1': [40, 28], 'input_seq_2': [40, 28],
                     'input_add_1': [4], 'input_add_2': [4], 'input_add_add': [3]})
    data_test = tf.data.Dataset.from_generator(lambda: generator_test(new_data, config['MGF_PATH']),
                                               output_types=({'input_spec': tf.float64, 'input_seq_1': tf.float64, 'input_seq_2': tf.float64,
                                                              'input_add_1': tf.float64, 'input_add_2': tf.float64, 'input_add_add': tf.float64}),
                                               output_shapes=output_shape)

    data_test = data_test.batch(config['BATCH'])
